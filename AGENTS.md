...
- Feature "Redefinição de senha": fluxo antigo por e-mail (SMTP) foi **removido**. Agora apenas o admin redefine a senha de um usuário via `POST /api/users/:id/reset-password` (painel `/admin`, botão "Resetar senha" — define senha temporária). O backend marca `users.must_change_password = true` e o usuário cria uma nova senha no próximo login via `POST /api/auth/change-password` (página `/change-password`). Coluna `must_change_password` garantida em `ensureSchema.js`.
- Rankings de competição (placar ao vivo, final e histórico) usam o mesmo critério: `media = score / total_answered`, ordenados por `media DESC, correct_answers DESC, score DESC`. Pontos por resposta correta: `1000 × (0.5 + 0.5 × (1 − tempo/tempoLimite))`; erro = 0 pontos.
